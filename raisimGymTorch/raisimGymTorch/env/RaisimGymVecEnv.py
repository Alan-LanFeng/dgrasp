...
class RaisimGymVecEnv:
    ...
    def __init__(self, impl, cfg, normalize_ob=False,
        ...
        self.object_sample_num = 200

        if self.get_pcd:
            self.obj_pcd = np.zeros([self.num_envs, self.object_sample_num, 3])
            # self.obj_normal = np.zeros([self.num_envs,500,3])
            if self.num_envs == 1: # if is in test mode
                ...
                sampled_points, face_id = trimesh.sample.sample_surface(obj_mesh, self.obj_pcd.shape[1])
                ...
                for cnt,obj_name in enumerate(np.unique(label['obj_name'])):
                    ...
                    sampled_normals = np.repeat(sampled_normals[np.newaxis, ...], obj_num, 0)
    ...
    def move_to_first(self,i):
        ...
        self.obj_pcd = np.zeros([self.num_envs, self.object_sample_num, 3])
        self.obj_normal = np.zeros([self.num_envs, self.object_sample_num, 3])
        self.obj_mesh = {}
        obj_name = label['obj_name'][0]
        p = f'../rsc/meshes_simplified/{obj_name}/textured_simple.obj'
        # obj_pcd = get_obj_pcd(p,num_p=3000)
        obj_mesh = trimesh.load_mesh(p)
        sampled_points, face_id = trimesh.sample.sample_surface(obj_mesh, self.object_sample_num)
        sampled_normals = obj_mesh.face_normals[face_id]
