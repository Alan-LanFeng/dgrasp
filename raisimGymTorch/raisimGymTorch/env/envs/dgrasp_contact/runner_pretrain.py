...
from raisimGymTorch.env.bin import dgrasp_contact as mano
...
from raisimGymTorch.env.bin.dgrasp_contact import NormalSampler
import os
import time
...
def get_ppo(mod):
    actor = ppo_module.Actor(mod_act,
                             ppo_module.MultivariateGaussianDiagonalCovariance(act_dim, num_envs, 3.0,
...
repeated_label['final_contacts'][:,1:]=1
...
saver = ConfigurationSaver(log_dir=exp_path + "/raisimGymTorch/" + args.storedir + "/" + args.exp_name,
                           save_items=[task_path + "/cfgs/" + args.cfg, task_path + "/Environment.hpp",
                                       task_path + "/runner.py"], test_dir=False)
